...
do
    for block in "${blocks[@]}"
    do
        echo -e "${blue}Matrix size: $size, Blocks: $block ${end_color}"
        echo -e "${blue}Executing test...${end_color}"
        OUTPUT=`./matrix_multiplication input_matrices/a_${size}_matrix.txt input_matrices/b_${size}_matrix.txt output_matrices/c_${size}_matrix.txt ${size} ${block}`
        echo -e ${OUTPUT}
        echo -e ${OUTPUT} >> out/"cuda_shared_memory_size_${size}_blocks_${block}.out"
        echo -e "${green}Done${end_color}"
        echo -e 
    done
done
